...
phone_pattern2 = r'(?:\()?(?P<area>\d{3})(?:\))?\s*(?P<prefix>\d{3})-(?P<number>\d{4})'  # 不捕获括号
...
for i, match in enumerate(re.finditer(phone_pattern2, phone_text)):
    print(f"  命名分组 - 电话{i+1}: {match.groupdict()}")
```
...
print(f"  分析文本: {extract_text.strip()[:50]}...")

for info_type, pattern in extraction_patterns.items():
...
1. **转义字符**：在Python字符串中使用原始字符串（r''）
2. **性能考虑**：复杂正则表达式可能影响性能
3. **可读性**：过于复杂的正则表达式难以维护
4. **边界情况**：考虑空字符串、特殊字符等情况
5. **安全性**：避免正则表达式注入攻击
