...
def demonstrate_practical_examples():
    ...
    def convert_file_encoding(content, from_encoding, to_encoding):
        """转换文件编码"""
        try:
            # 如果输入是字符串，先用from_encoding编码
            ...
            # 解码
            text_content = byte_content.decode(from_encoding)
            
            # 重新编码
            result = text_content.encode(to_encoding)
    ...
    print(f"测试内容: {repr(test_content)}")
    print(f"{'转换':<15} {'状态':<10} {'结果大小':<10}")
    print("-" * 40)
    
    for from_enc, to_enc in conversions:
        result, text, error = convert_file_encoding(test_content, from_enc, to_enc)
        if result:
            print(f"{from_enc}->{to_enc:<8} {'成功':<10} {len(result)}字节")
        else:
            print(f"{from_enc}->{to_enc:<8} {'失败':<10} {error[:10]}...")
